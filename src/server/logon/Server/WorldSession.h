...
class WorldSession
{
    public:
        ...
        bool GoToNode(uint32 ID);
        void SetConnector(NodeSocket *Socket) 
        { 
            CloseNodeSocket();
            m_redirected = true;
            m_NodeSocket = Socket; 
        }
        void CloseNodeSocket();
        void SetNodeID(uint32 nodeid) { m_NodeID = nodeid; }
        //Should be enabled later
        void SetReconnect(bool sure){}
        ...
        void SendPacket(WorldPacket const* packet, bool nocheck = false);
        ...
        void QueueNodePacket(WorldPacket* new_packet);
        
        bool Update(uint32 diff, PacketFilter& updater);
        bool UpdateClientIO(uint32 diff, PacketFilter& updater);
        bool UpdateNodeIO(uint32 diff, PacketFilter& updater);
    ...
    private:
        ...
        ACE_Based::LockedQueue<WorldPacket*, ACE_Thread_Mutex> _recvNodeQueue;
