...
WorldSession::~WorldSession()
{
    ...
    if (m_NodeSocket)
    {
        m_NodeSocket->CloseSocket ();
        m_NodeSocket->RemoveReference ();
        m_NodeSocket = NULL;
    }
    /// - If have unclosed socket, close it
    if (m_Socket)
...
void WorldSession::CloseNodeSocket()
{
    if (m_NodeSocket || m_NodeSocket != NULL )
    {
        m_NodeSocket->RemoveReference();
        m_NodeSocket->CloseSocket();
        m_NodeSocket = NULL;
...
void WorldSession::SendPacketToNode(WorldPacket const *packet)
{
    ...
    if (m_NodeSocket->SendPacket (*packet) == -1)    
        m_NodeSocket->CloseSocket ();
}
...
void WorldSession::QueueNodePacket(WorldPacket *new_packet)
{
    _recvNodeQueue.add(new_packet);
}
...
bool WorldSession::GoToNode(uint32 ID)
{
    uint32 node_id = sRoutingHelper->GetNodeForMap(ID);
    if (node_id=0)
        return false;

    if (sNodeSocketMgr->OpenConnection(node_id, this))
    {
        m_redirected = true;
        return true;
    }
