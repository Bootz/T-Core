...
class DatabaseWorkerPool
{
    public:
        ...
        bool Open(const std::string& infoString, uint8 async_threads, uint8 synch_threads)
        {
            ...
            sLogMgr->WriteLn(SQLDRIVER_LOG, "Opening databasepool '%s'. Async threads: %u, synch threads: %u", m_connectionInfo.database.c_str(), async_threads, synch_threads);
            ...
            sLogMgr->WriteLn(SQLDRIVER_LOG, "Databasepool opened succesfuly. %u total connections running.", (m_connectionCount[IDX_SYNCH] + m_connectionCount[IDX_ASYNC]));
            return res;
        }

        void Close()
        {
            sLogMgr->WriteLn(SQLDRIVER_LOG, "Closing down databasepool '%s'.", m_connectionInfo.database.c_str());
            ...
            sLogMgr->WriteLn(SQLDRIVER_LOG, "Asynchronous connections on databasepool '%s' terminated. Proceeding with synchronous connections.", m_connectionInfo.database.c_str());
            ...
            sLogMgr->WriteLn(SQLDRIVER_LOG, "All connections on databasepool %s closed.", m_connectionInfo.database.c_str());
        }
        ...
        */
        ...
        void CommitTransaction(SQLTransaction transaction)
        {
            if (sLogMgr->IsLogEnabled(SQLDRIVER_LOG))
            {
                ...
                {
                    case 0:
                        sLogMgr->WriteLn(SQLDRIVER_LOG, "Transaction contains 0 queries. Not executing.");
                        return;
                    case 1:
                        sLogMgr->WriteLn(SQLDRIVER_LOG, "[WARNING] Transaction only holds 1 query, consider removing Transaction context in code.");
                        break;
                    default:
                        break;
                }
