...
void LoadDataStores(const std::string& dataPath)
{
    ...
    //LoadData(availableDb2Locales, bad_db2_files, sItemSparseStore, db2Path, "Item-sparse.db2");
    ...
    LoadData(availableDbcLocales, bad_dbc_files, sAchievementStore,            dbcPath, "Achievement.dbc"); //;&CustomAchievementfmt, &CustomAchievementIndex);
...
//    LoadData(availableDbcLocales, bad_dbc_files, sGtOCTClassCombatRatingScalarStore,    dbcPath, "gtOCTClassCombatRatingScalar.dbc");

    //LoadData(dbcCount, availableDbcLocales, bad_dbc_files, sGtOCTRegenMPStore,           dbcPath, "gtOCTRegenMP.dbc");       -- not used currently
//    LoadData(availableDbcLocales, bad_dbc_files, sGtRegenHPPerSptStore,        dbcPath, "gtRegenHPPerSpt.dbc");
    ...
    //LoadData(availableDbcLocales, bad_dbc_files, sStableSlotPricesStore,       dbcPath, "StableSlotPrices.dbc");
    ...
    {
        sLog->outError("Incorrect DataDir value in worldserver.conf or ALL required *.dbc files (%d) not found by path: %sdbc", DataFileCount, dataPath.c_str());
        exit(1);
    }
    ...
    {
        sLog->outError("You have _outdated_ Storages files. Please extract correct versions from current using client.");
        exit(1);
    }

    sLog->outString(">> Initialized %d data storages in %u ms", DataFileCount, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
