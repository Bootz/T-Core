...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    if ((m_spellInfo->AttributesEx3 & SPELL_ATTR3_PLAYERS_ONLY) || (TargetType == SPELL_TARGETS_ENTRY && !entry))
    {
        ...
        TagUnitMap.remove_if(Trillium::ObjectTypeIdCheck(TYPEID_PLAYER, false)); // above line will select also pets and totems, remove them
    }
    else
...
void Spell::SelectEffectTargets(uint32 i, SpellImplicitTargetInfo const& cur)
{
    ...
    if (pushType == PUSH_CHAIN) // Chain
    {
        ...
        if (maxTargets > 1)
        {
            ...
            CallScriptAfterUnitTargetSelectHandlers(unitList, (SpellEffectEntry*)m_spellInfo->Effects[i].IsEffect());
...
void Spell::_handle_immediate_phase()
{
    ...
    if (oldEffMask != m_effectMask && m_UniqueTargetInfo.empty())
    {
        ...
        if (!procAttacker)
        {
            ...
            for (uint8 i = 0; i< MAX_SPELL_EFFECTS; ++i)
                // If at least one effect negative spell is negative hit
                if (m_effectMask & (1<<i) && !m_spellInfo->IsPositive())
                {
                    positive = false;
                    break;
                }
...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER && m_timer != 0) &&
        ...
        (m_spellInfo->Effects[EFFECT_0].Effect != SPELL_EFFECT_STUCK || !m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING)))
    {
...
SpellCastResult Spell::CheckCast(bool strict)
{
        ...
        else if (m_caster == target)
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER) // Target - is player caster
            {
                ...
                if (m_targets.GetTargetMask() == TARGET_FLAG_SELF &&
                    m_spellInfo->Effects[EFFECT_1].TargetA.GetTarget() == TARGET_UNIT_TARGET_ENEMY)
                {
    ...
    if ((m_spellInfo->AttributesEx4 & SPELL_ATTR4_NOT_USABLE_IN_ARENA) ||
        (m_spellInfo->RecoveryTime > 10 * MINUTE * IN_MILLISECONDS && !(m_spellInfo->AttributesEx4 & SPELL_ATTR4_USABLE_IN_ARENA)))
    ...
    if (m_spellInfo->AttributesEx & SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY)
    {
        for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (m_spellInfo->Effects[i].ApplyAuraName == SPELL_AURA_SCHOOL_IMMUNITY)
                ...
                dispel_immune |= m_spellInfo->GetDispelMask(DispelType(m_spellInfo->Effects[i].MiscValue));
        }
...
bool Spell::CheckTargetCreatureType(Unit* target) const
{
    ...
    if (target->GetEntry() == 5925 && m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && (m_spellInfo->SpellFamilyFlags[0] & 0x1000000) && m_spellInfo->Effects[EFFECT_0].ApplyAuraName == SPELL_AURA_MOD_CONFUSE)
        return true;
...
bool Spell::IsValidSingleTargetSpell(Unit const* target) const
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (!IsValidSingleTargetEffect(target, Targets(m_spellInfo->Effects[i].TargetA.GetTarget())))
            return false;
        // Need to check B?
        //if (!IsValidSingleTargetEffect(m_spellInfo->Effects[i].TargetA, target)
        //    return false;
    }
    return true;
}
        ...
        if (effectMask & (1<<i))
        {
            ...
            switch(m_spellInfo->Effects[i].Effect)
            {
                ...
                case SPELL_EFFECT_WEAPON_PERCENT_DAMAGE:
                    SpellDamageWeaponDmg((SpellEffectEntry*)m_spellInfo->Effects[i].Effect);
                    break;
        ...
        switch(lockInfo->Type[j])
        {
            ...
            case LOCK_KEY_SKILL:
            {
                ...
                if (skillId != SKILL_NONE)
                {
                    ...
                    if (m_spellInfo->Effects[effIndex].TargetA.GetTarget() == TARGET_GAMEOBJECT_ITEM || m_spellInfo->Effects[effIndex].TargetA.GetTarget() == TARGET_GAMEOBJECT_ITEM)
