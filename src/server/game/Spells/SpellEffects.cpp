...
void Spell::EffectResurrectNew(SpellEffIndex effIndex)
{
    ...
    uint32 mana = m_spellInfo->GetEffectMiscValue(effIndex);
...
void Spell::SpellDamageSchoolDmg(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        switch (m_spellInfo->GetSpellFamilyName())
        {
            ...
            {
                // Incinerate Rank 1 & 2
                if ((m_spellClass->SpellFamilyFlags[1] & 0x000040) && m_spellInfo->SpellIconID == 2128)
                {
                ...
                else if (m_spellInfo->GetTargetAuraState() == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin(); i != mPeriodic.end(); ++i)
                    {
                        ...
                        if ((*i)->GetSpellClass()->SpellFamilyName != SPELLFAMILY_WARLOCK ||
                            (*i)->GetCasterGUID() != m_caster->GetGUID())
                            continue;

                        // Immolate
                        if ((*i)->GetSpellClass()->SpellFamilyFlags[0] & 0x4)
                        {
                        ...
                        // Shadowflame
                        if ((*i)->GetSpellClass()->SpellFamilyFlags[2] & 0x00000002)
                ...
                // Shadow Bite
                else if (m_spellClass->SpellFamilyFlags[1] & 0x400000)
                {
            ...
            case SPELLFAMILY_PRIEST:
            {
                ...
                if ((m_spellClass->SpellFamilyFlags[1] & 0x2))
                {
                ...
                else if (m_caster->GetShapeshiftForm() == FORM_SHADOW && (m_spellClass->SpellFamilyFlags[0] & 0x00002000))
                {
                    Unit::AuraEffectList const& ImprMindBlast = m_caster->GetAuraEffectsByType(SPELL_AURA_ADD_FLAT_MODIFIER);
                    for (Unit::AuraEffectList::const_iterator i = ImprMindBlast.begin(); i != ImprMindBlast.end(); ++i)
                    {
                        if ((*i)->GetSpellClass()->SpellFamilyName == SPELLFAMILY_PRIEST &&
            ...
            {
                // Ferocious Bite
                if (m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellClass->SpellFamilyFlags[0] & 0x000800000) && m_spellInfo->SpellVisual[0] == 6587)
                {
                    // converts each extra point of energy into ($f1+$AP/410) additional damage
                    float ap = m_caster->GetTotalAttackPowerValue(BASE_ATTACK);
                    float multiple = ap / 410 + m_spellEffect->EffectDamageMultiplier;
                    int32 energy = -(m_caster->ModifyPower(POWER_ENERGY, -30));
                    damage += int32(energy * multiple);
                    damage += int32(CalculatePctN(m_caster->ToPlayer()->GetComboPoints() * ap, 7));
                }
                // Wrath
                else if (m_spellClass->SpellFamilyFlags[0] & 0x00000001)
                {
            ...
            {
                // Envenom
                if (m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellClass->SpellFamilyFlags[1] & 0x8))
                {
                    ...
                    if (uint32 combo = m_caster->ToPlayer()->GetComboPoints())
                    {
                        ...
                        if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x10000, 0, 0, m_caster->GetGUID()))
                        {
                            ...
                            {
                                if ((*iter)->GetSpellClass()->SpellFamilyName == SPELLFAMILY_ROGUE && (*iter)->GetSpellProto()->SpellIconID == 1960)
                                {
                ...
                // Eviscerate
                else if ((m_spellClass->SpellFamilyFlags[0] & 0x00020000) && m_caster->GetTypeId() == TYPEID_PLAYER)
                {
            ...
            case SPELLFAMILY_HUNTER:
            {
                ...
                // Steady Shot
                else if (m_spellClass->SpellFamilyFlags[1] & 0x1)
                {
                    bool found = false;
                    // check dazed affect
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (m_spellClass->SpellFamilyFlags[1]&0x00040000)
                {
                ...
                if (m_spellClass->SpellFamilyFlags[EFFECT_1] & 0x100000)
                {
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                if (m_spellClass->SpellFamilyFlags[0] & 0x00040000)
                {
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_WARRIOR:
            // Charge
            if (m_spellClass->SpellFamilyFlags & SPELLFAMILYFLAG_WARRIOR_CHARGE && m_spellInfo->SpellVisual[0] == 867)
            {
            ...
            // Slam
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_WARRIOR_SLAM && m_spellInfo->SpellIconID == 559)
            {
                int32 bp0 = damage;
            ...
            // Execute
            if (m_spellClass->SpellFamilyFlags[EFFECT_0] & SPELLFAMILYFLAG_WARRIOR_EXECUTE)
            {
                if (!unitTarget)
                    return;
                ...
                bp = damage + int32(rageUsed * m_spellEffect->EffectDamageMultiplier + m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.2f);
                break;
            }
            // Concussion Blow
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_WARRIOR_CONCUSSION_BLOW)
            {
        ...
        case SPELLFAMILY_WARLOCK:
            // Life Tap
            if ((m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_WARLOCK_LIFETAP) && m_caster->ToPlayer())
            {
                float spFactor = 0.0f;
                switch (m_spellInfo->Id)
                {
        ...
        case SPELLFAMILY_DRUID:
            // Starfall
            if (m_spellClass->SpellFamilyFlags[2] & SPELLFAMILYFLAG2_DRUID_STARFALL)
            {
        ...
        case SPELLFAMILY_PALADIN:
            // Divine Storm
            if (m_spellClass->SpellFamilyFlags[1] & SPELLFAMILYFLAG1_PALADIN_DIVINESTORM && effIndex == 1)
            {
        ...
        case SPELLFAMILY_SHAMAN:
            ...
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_SHAMAN_TOTEM_EFFECTS && m_spellInfo->SpellIconID == 1673)
            {
                int32 bp1 = 1;
            ...
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_SHAMAN_HEALING_STREAM)
            {
                if (!unitTarget)
                    return;
                // Restorative Totems
            ...
            // Mana Spring Totem
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_SHAMAN_MANA_SPRING)
            {
            ...
            // Lava Lash
            if (m_spellClass->SpellFamilyFlags[2] & SPELLFAMILYFLAG2_SHAMAN_LAVA_LASH)
            {
                if (m_caster->GetTypeId() != TYPEID_PLAYER)
                    return;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            // Death strike
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_DK_DEATH_STRIKE)
            {
            ...
            // Death Coil
            if (m_spellClass->SpellFamilyFlags[0] & SPELLFAMILYFLAG_DK_DEATH_COIL)
            {
...
{
    if (m_spellInfo->GetEffectMiscValue(effIndex) < 0 || m_spellInfo->GetEffectMiscValue(effIndex) >= int8(MAX_POWERS))
        return;

    Powers powerType = Powers(m_spellInfo->GetEffectMiscValue(effIndex));
    ...
    {
        gainMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(m_spellEffect, effIndex, m_originalCaster, this);
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
    ...
    */
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell ScriptStart %u for spellid %u in EffectSendEvent ", m_spellInfo->GetEffectMiscValue(effIndex), m_spellInfo->Id);
    ...
    if (unitTarget)
    {
        if (ZoneScript* zoneScript = unitTarget->GetZoneScript())
            zoneScript->ProcessEvent(unitTarget, m_spellInfo->GetEffectMiscValue(effIndex));
    }

    m_caster->GetMap()->ScriptsStart(sEventScripts, m_spellInfo->GetEffectMiscValue(effIndex), m_caster, pTarget);
}
...
void Spell::EffectPowerBurn(SpellEffIndex effIndex)
{
    ...
    float dmgMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(m_spellEffect, effIndex, m_originalCaster, this);
...
void Spell::SpellDamageHeal(SpellEffIndex /*effIndex*/)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->GetTargetAuraState() == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND, m_spellInfo, m_caster))
        {
            ...
            {
                if ((*i)->GetSpellClass()->SpellFamilyName == SPELLFAMILY_DRUID
                    && (*i)->GetSpellClass()->SpellFamilyFlags[0] & 0x50)
                {
            ...
            int32 tickcount = 0;
            // Rejuvenation
            if (targetAura->GetSpellClass()->SpellFamilyFlags[0] & 0x10)
                tickcount = 4;
            // Regrowth
        ...
        // Glyph of Nourish
        else if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_DRUID && m_spellClass->SpellFamilyFlags[1] & 0x2000000)
        {
        ...
        else if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && m_spellClass->SpellFamilyFlags[0] & 0x100)
        {
            addhealth = caster->SpellHealingBonus(unitTarget, m_spellInfo, addhealth, HEAL);
        ...
        else if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT && m_spellClass->SpellFamilyFlags[0] & 0x00080000)
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    float healMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(m_spellEffect, effIndex, m_originalCaster, this);
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
    if (!m_spellAura)
    {
        float radius = GetSpellRadiusForFriend(sSpellRadiusStore.LookupEntry(m_spellInfo->GetEffectRadiusIndex(effIndex)));
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    Powers power = Powers(m_spellInfo->GetEffectMiscValue(effIndex));
    ...
    switch (properties->Category)
    {
        ...
        case SUMMON_CATEGORY_UNK:
            ...
            switch (properties->Type)
            {
                ...
                default:
                {
                    float radius = GetSpellRadiusForHostile(sSpellRadiusStore.LookupEntry(m_spellInfo->GetEffectRadiusIndex(effIndex)));
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
    ...
    uint32 spellToLearn = (m_spellInfo->Id == 483 || m_spellInfo->Id == 55884) ? damage : m_spellInfo->GetEffectTriggerSpell(effIndex);
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
    ...
    for (int32 count = 0; count < damage && !dispel_list.empty();)
    {
        ...
        bool success = false;
        // 2.4.3 Patch Notes: "GetDispel() effects will no longer attempt to remove effects that have 100% dispel resistance."
...
void Spell::EffectTeleUnitsFaceCaster(SpellEffIndex effIndex)
{
    ...
    float dis = (float)m_caster->GetSpellRadiusForTarget(unitTarget, sSpellRadiusStore.LookupEntry(m_spellInfo->GetEffectRadiusIndex(effIndex)));
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Trillium::Honor::hk_honor_at_level(unitTarget->getLevel(), float(damage));
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (!itemTarget)
    return;
    if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && m_spellClass->SpellFamilyFlags[0] & 0x400000)
    {
        uint32 spell_id = 0;
...
void Spell::EffectLearnPetSpell(SpellEffIndex effIndex)
{
    ...
    SpellEntry const *learn_spellproto = sSpellStore.LookupEntry(m_spellInfo->GetEffectTriggerSpell(effIndex));
    if (!learn_spellproto)
        return;
...
void Spell::SpellDamageWeaponDmg(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (m_spellClass->SpellFamilyFlags[1] & 0x40)
            {
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            if ((m_spellClass->SpellFamilyFlags[1] & 0x40000)
                || (m_spellClass->SpellFamilyFlags[0] & 0x6000000))
            {
                // Hemorrhage
                if (m_spellClass->SpellFamilyFlags[0] & 0x2000000)
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
            ...
            else if (m_spellClass->SpellFamilyFlags[1] & 0x6)
            {
                bool found = false;
                // fast check
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            else if (m_spellClass->SpellFamilyFlags[0] & 0x00008800 && unitTarget->HasAuraState(AURA_STATE_BLEEDING))
            {
...
void Spell::EffectHealMaxHealth(SpellEffIndex /*effIndex*/)
{
    ...
    int32 addhealth;
    if (m_spellInfo->GetSpellFamilyName() == SPELLFAMILY_PALADIN) // Lay on Hands
    {
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex)
{
    ...
    // apply new one
    if (uint32 glyph = m_spellInfo->GetEffectMiscValue(effIndex))
    {
...
void Spell::EffectEnchantHeldItem(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->GetEffectMiscValue(effIndex))
    {
        uint32 enchant_id = m_spellInfo->GetEffectMiscValue(effIndex);
...
void Spell::EffectFeedPet(SpellEffIndex effIndex)
{
    ...
    m_caster->CastCustomSpell(pet, m_spellInfo->GetEffectTriggerSpell(effIndex), &benefit, NULL, NULL, true);
}
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    else if (m_spellInfo->GetEffectRadiusIndex(effIndex) && m_spellInfo->speed == 0)
    {
        float dis = GetSpellRadiusForFriend(sSpellRadiusStore.LookupEntry(m_spellInfo->GetEffectRadiusIndex(effIndex)));
...
void Spell::EffectStealBeneficialBuff(SpellEffIndex effIndex)
{
    ...
    // Create dispel mask by dispel type
    uint32 dispelMask  = GetDispellMask(DispelType(m_spellInfo->GetEffectMiscValue(effIndex)));
    Unit::AuraMap const& auras = unitTarget->GetOwnedAuras();
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        Aura * aura = itr->second;
...
void Spell::EffectKillCreditPersonal(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->KilledMonsterCredit(m_spellInfo->GetEffectMiscValue(effIndex), 0);
}
...
void Spell::EffectQuestFail(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->FailQuest(m_spellInfo->GetEffectMiscValue(effIndex));
}
...
void Spell::EffectQuestStart(SpellEffIndex effIndex)
{
    ...
    if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(m_spellInfo->GetEffectMiscValue(effIndex)))
    {
...
void Spell::EffectActivateRune(SpellEffIndex effIndex)
{
    ...
    {
        if (plr->GetRuneCooldown(j) && plr->GetCurrentRune(j) == RuneType(m_spellInfo->GetEffectMiscValue(effIndex)))
        {
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const *properties)
{
    ...
    //float radius = GetSpellRadiusForFriend(sSpellRadiusStore.LookupEntry(m_spellInfo->GetEffectRadiusIndex(i)));
    float radius = 5.0f;
    uint32 amount = damage > 0 ? damage : 1;
    int32 duration = GetSpellDuration(m_spellInfo);
    switch (m_spellInfo->Id)
...
void Spell::EffectRemoveAura(SpellEffIndex effIndex)
{
    ...
    unitTarget->RemoveAurasDueToSpell(m_spellInfo->GetEffectTriggerSpell(effIndex));
}
...
void Spell::EffectCastButtons(SpellEffIndex effIndex)
{
    ...
    uint32 button_id = m_spellInfo->GetEffectMiscValue(effIndex) + 132;
    uint32 n_buttons = m_spellInfo->GetEffectMiscValueB(effIndex);
