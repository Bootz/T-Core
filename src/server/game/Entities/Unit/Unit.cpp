...
bool Unit::HasAuraTypeWithFamilyFlags(AuraType auraType, uint32 familyName, uint32 familyFlags) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if (SpellClassOptionsEntry const* iterSpellProto = (*itr)->GetSpellClass())
...
void Unit::CastSpell(GameObject *go, uint32 spellId, bool triggered, Item *castItem, AuraEffect* triggeredByAura, uint64 originalCaster)
{
    ...
    SpellTargetRestrictionsEntry const* spellInfo1 = sSpellTargetRestrictionsStore.LookupEntry(spellId);
    if (!(spellInfo1->Targets & (TARGET_FLAG_OBJECT | TARGET_FLAG_OBJECT_CASTER)))
    {
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    if (IsDamageReducedByArmor(damageSchoolMask, spellInfo))
        damage = CalcArmorReducedDamage(pVictim, damage, spellInfo1, attackType);
...
bool Unit::IsDamageReducedByArmor(SpellSchoolMask schoolMask, SpellEntry const *spellInfo, uint8 effIndex)
{
    ...
    if (spellInfo)
    {
        ...
        if (effIndex != MAX_SPELL_EFFECTS && spellInfo->GetEffectApplyAuraName(effIndex) == SPELL_AURA_PERIODIC_DAMAGE)
...
uint32 Unit::CalcArmorReducedDamage(Unit* pVictim, const uint32 damage, SpellClassOptionsEntry const *spellInfo, WeaponAttackType /*attackType*/)
{
    uint32 newdamage = 0;
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (AuraEffectList::const_iterator itr = ResIgnoreAuras.begin(); itr != ResIgnoreAuras.end(); ++itr)
        {
            ...
            for (int i = 0; i < MAX_ATTACK; ++i)
            {
                ...
                if (weapon && weapon->IsFitToSpellRequirements((*itr)->GetSpellEquipped()))
                {
                    armor = floor(AddPctN(armor, -(*itr)->GetAmount()));
                    break;
                }
...
void Unit::CalcAbsorbResist(Unit *pVictim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, SpellEntry const *spellInfo)
{
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0)
    {
        ...
        AuraEffectList const& ResIgnoreAurasAb = GetAuraEffectsByType(SPELL_AURA_MOD_ABILITY_IGNORE_TARGET_RESIST);
        for (AuraEffectList::const_iterator j = ResIgnoreAurasAb.begin(); j != ResIgnoreAurasAb.end(); ++j)
            if (((*j)->GetMiscValue() & schoolMask) && (*j)->IsAffectedOnSpell(spellInfo1))
    ...
    for (AuraEffectList::const_iterator itr = AbsIgnoreAurasB.begin(); itr != AbsIgnoreAurasB.end(); ++itr)
    {
        ...
        if (((*itr)->GetAmount() > auraAbsorbMod) && (*itr)->IsAffectedOnSpell(spellInfo1))
            auraAbsorbMod = float((*itr)->GetAmount());
    }
    ...
    vSchoolAbsorbCopy.sort(Trillium::AbsorbAuraOrderPred());
    ...
    for (AuraEffectList::const_iterator itr = vManaShieldCopy.begin(); (itr != vManaShieldCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
        ...
        if (float manaMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(absorbAurEff->GetSpellEffect(), absorbAurEff->GetEffIndex(), absorbAurEff->GetCaster()))
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    if (!IsPositiveSpell(spell->Id))
    {
        ...
        {
            if (spell->GetEffectApplyAuraName(i) != 0)
            {
                bNegativeAura = true;
                break;
            }
        ...
        if (bNegativeAura)
        {
            tmp += pVictim->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(spell->GetDispel())) * 100;
            tmp += pVictim->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(spell->GetDispel())) * 100;
        }
    }
        ...
        if (Aura* foundAura = GetOwnedAura(newAura->Id, casterGUID, (sSpellMgr->GetSpellCustomAttr(newAura->Id) & SPELL_ATTR0_CU_ENCHANT_PROC) ? castItemGUID : 0, 0))
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                if (baseAmount)
                    ...
                    bp = foundAura->GetSpellProto()->GetEffectBasePoints(i);
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            switch (aura->GetSpellProto()->GetSpellFamilyName())
            {
                case SPELLFAMILY_WARLOCK:
                {
                    ...
                    if (aura->GetSpellClass()->SpellFamilyFlags[1] & 0x0100)
                    {
                ...
                {
                    // Lifebloom
                    if (aura->GetSpellClass()->SpellFamilyFlags[1] & 0x10)
                    {
                ...
                {
                    // Flame Shock
                    if (aura->GetSpellClass()->SpellFamilyFlags[0] & 0x10000000)
                    {
                        if (Unit* caster = aura->GetCaster())
                        {
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, uint64 casterGUID)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        {
            SpellClassOptionsEntry const* spell = aura->GetSpellClass();
...
AuraEffect* Unit::GetAuraEffect(AuraType type, SpellFamilyNames name, uint32 iconId, uint8 effIndex) const
{
    ...
    for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        if (effIndex != (*itr)->GetEffIndex())
            continue;
        SpellClassOptionsEntry const* spell = (*itr)->GetSpellClass();
        SpellEntry const* spell1 = (*itr)->GetSpellProto();
        if (spell1->SpellIconID == iconId && spell->SpellFamilyName == uint32(name) && !spell->SpellFamilyFlags)
            return *itr;
    }
    return NULL;
}
...
AuraEffect* Unit::GetAuraEffect(AuraType type, SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, uint64 casterGUID)
{
    ...
    {
        SpellClassOptionsEntry const* spell = (*i)->GetSpellClass();
        if (spell->SpellFamilyName == uint32(family) && spell->SpellFamilyFlags.HasFlag(familyFlag1, familyFlag2, familyFlag3))
        {
...
{
    SpellClassOptionsEntry const* affectedSpell1 = NULL;
    AuraEffectList const& mTotalAuraList = GetAuraEffectsByType(auratype);
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
        if ((*i)->IsAffectedOnSpell(affectedSpell1))
            return true;
    return false;
}
...
bool Unit::HasAuraWithMechanic(uint32 mechanicMask)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end(); ++iter)
    {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (iter->second->HasEffect(i) && spellInfo->GetSpellEffectIdByIndex(i) && spellInfo->GetEffectMechanic(i))
                if (mechanicMask & (1 << spellInfo->GetEffectMechanic(i)))
                    return true;
    }
...
int32 Unit::GetMaxPositiveAuraModifierByAffectMask(AuraType auratype, SpellEntry const* affectedSpell) const
{
    ...
    {
        SpellClassOptionsEntry const* affectedSpell1 = NULL;
        if ((*i)->IsAffectedOnSpell(affectedSpell1) && (*i)->GetAmount() > modifier)
            modifier = (*i)->GetAmount();
    }
...
bool Unit::HandleDummyAuraProc(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    SpellClassOptionsEntry const* dummyClass = triggeredByAura->GetSpellClass();
    SpellEffectEntry const* procEffect = triggeredByAura->GetSpellEffect();
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            //    int32 cost = int32(procSpell->GetManaCost() + CalculatePctU(GetCreateMana(), procSpell->ManaCostPercentage));
            //    basepoints0 = CalculatePctN(cost, triggerAmount);
            //    if (basepoints0 <= 0)
            //        return false;
            ...
            switch(dummySpell->Id)
            {
                ...
                case 56372:
                {
                    ...
                    {
                        SpellClassOptionsEntry const* cdSpell = sSpellClassOptionsStore.LookupEntry(itr->first);
                        // Frost Nova
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            // Retaliation
            if (dummyClass->SpellFamilyFlags[1] & 0x8)
            {
        ...
        {
            // Seed of Corruption
            if (dummyClass->SpellFamilyFlags[1] & 0x00000010)
            {
                if (procSpell && dummyClass->SpellFamilyFlags[1] & 0x8000)
                    return false;
            ...
            if (dummyClass->SpellFamilyFlags.IsEqual(0, 0, 0) && dummySpell->SpellIconID == 1932)
            {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 15286:
                {
                    if (!pVictim || !pVictim->isAlive() || dummyClass->SpellFamilyFlags[1] & 0x80000)
                        return false;
                ...
                {
                    // Shadow Word: Pain
                    if (dummyClass->SpellFamilyFlags[0] & 0x8000)
                        triggered_spell_id = 40441;
                    // Renew
                    else if (dummyClass->SpellFamilyFlags[0] & 0x40)
                ...
                case 70770:
                    // Flash Heal
                    if (dummyClass->SpellFamilyFlags[0] & 0x800)
                    {
        ...
        case SPELLFAMILY_DRUID:
        {
            switch(dummySpell->Id)
            {
                ...
                case 54821:
                {
                    if (procSpell->SpellVisual[0] == 750 && procSpell->GetEffectApplyAuraName(1) == 3)
                    {
                ...
                case 40442:
                {
                    ...
                    // Starfire
                    if (dummyClass->SpellFamilyFlags[0] & 0x4)
                    {
                        triggered_spell_id = 40445;
                        chance = 25.0f;
                    }
                    // Rejuvenation
                    else if (dummyClass->SpellFamilyFlags[0] & 0x10)
                    {
                        triggered_spell_id = 40446;
                        chance = 25.0f;
                    }
                    // Mangle (Bear) and Mangle (Cat)
                    else if (dummyClass->SpellFamilyFlags[1] & 0x00000440)
                    {
                ...
                {
                    // Wrath & Starfire
                    if ((dummyClass->SpellFamilyFlags[0] & 0x5) && (procEx & PROC_EX_CRITICAL_HIT))
                    {
            ...
            else if (dummySpell->SpellIconID == 2850)
            {
                ...
                if (effIndex == 0)
                {
                    if (!(dummyClass->SpellFamilyFlags[0] & 0x00080000))
                        return false;
                ...
                {
                    if (!(dummyClass->SpellFamilyFlags[2] & 0x00000800))
                        return false;
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->SpellIconID == 2236)
            {
                ...
                // Explosive Shot
                if (dummyClass->SpellFamilyFlags[2] & 0x200)
                {
            ...
            if (dummySpell->SpellIconID == 3560)
            {
                ...
                if (!(dummyClass->SpellFamilyFlags[1] & 0x01000000))
                    return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummyClass->SpellFamilyFlags[0] & 0x8000000)
            {
                if (effIndex != 0)
                    return false;
            ...
            // Sacred Shield
            if (dummyClass->SpellFamilyFlags[1] & 0x80000)
            {
                if (procFlag & PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS)
                {
                    if (procSpell->GetSpellFamilyName() == SPELLFAMILY_PALADIN && (dummyClass->SpellFamilyFlags[0] & 0x40000000))
                    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31801:
                {
                    ...
                    bool damager = procSpell && procSpell->GetEquippedItemClass() == ITEM_CLASS_WEAPON;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    SpellEffectEntry const* windfurySpellEntry = sSpellEffectStore.LookupEntry(spellId);
            ...
            if (dummySpell->Id == 17619)
            {
                if (procSpell->GetSpellFamilyName() == SPELLFAMILY_POTION)
                {
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                    {
                        ...
                        basepoints0 = int32(CalculateSpellDamage(this, procEffect, i) * 0.4f);
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->SpellIconID == 3021)
            {
                ...
                else if ((procClass->SpellFamilyFlags[0] & 0x200000 || procClass->SpellFamilyFlags[1] & 0x10000) && !(procEx & PROC_EX_CRITICAL_HIT))
                    *handled = true;
                break;
            }
            ...
            case SPELLFAMILY_WARLOCK:
            {
                ...
                if (spellClass->SpellFamilyFlags[0] & 0x4000)
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator i = mAddFlatModifier.begin(); i != mAddFlatModifier.end(); ++i)
                    {
                        ...
                        {
                            int32 value2 = CalculateSpellDamage(this, (*i)->GetSpellEffect(), 2);
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 4993:
            {
                ...
                // effect 0 m_amount
                int32 stepPercent = CalculateSpellDamage(this, (*i)->GetSpellEffect(), 0);
...
uint32 Unit::SpellHealingBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    {
        // Gift of the Naaru
        if (SpellClass->SpellFamilyFlags[2] & 0x80000000 && spellProto->SpellIconID == 329)
        {
            scripted = true;
        ...
        else if (spellProto->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && SpellClass->SpellFamilyFlags[1] & 0x80000)
            factorMod *= 0.45f;
    ...
    // Nourish cast
    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && SpellClass->SpellFamilyFlags[1] & 0x2000000)
    {
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellEffectEntry const* spellProto, uint8 effect_index, int32 const* basePoints) const
{
    SpellEntry const* spellProto1 = NULL;
    return SpellMgr::CalculateSpellEffectAmount(spellProto1, effect_index, this, basePoints, target);
}
    ...
    if (target == this)
    {
        switch (spellProto->GetSpellFamilyName())
        {
            case SPELLFAMILY_DRUID:
                ...
                break;
            case SPELLFAMILY_PALADIN:
                if (spellClass->SpellFamilyFlags[0] & 0x00000002)
                {
                    // Glyph of Blessing of Might
                    if (AuraEffect* aurEff = GetAuraEffect(57958, 0))
                        duration += aurEff->GetAmount() * MINUTE * IN_MILLISECONDS;
                }
                else if (spellClass->SpellFamilyFlags[0] & 0x00010000)
                {
...
void Unit::ModSpellCastTime(SpellEntry const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (!(spellProto->Attributes & (SPELL_ATTR0_UNK4|SPELL_ATTR0_TRADESPELL)) && spellProto->GetSpellFamilyName())
...
Unit* Unit::SelectNearbyTarget(float dist) const
{
    ...
    Trillium::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    Trillium::UnitListSearcher<Trillium::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit *pVictim, Aura * aura, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const *& spellProcEvent)
{
    ...
    if (spellProcEvent && spellProcEvent->procFlags) // if exist get custom spellProcEvent->procFlags
        ...
        EventProcFlag = spellProcEvent->procFlags;       // else get from spell proto
    ...
    if (procSpell && procSpell->Id == spellProto->Id
        && !(spellProcEvent->procFlags & (PROC_FLAG_TAKEN_PERIODIC | PROC_FLAG_KILL)))
        return false;
...
void Unit::UpdateObjectVisibility(bool forced)
{
    ...
    else
    {
        ...
        Trillium::AIRelocationNotifier notifier(*this);
...
bool Unit::SetPosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!Trillium::IsValidMapCoord(x, y, z, orientation))
    {
