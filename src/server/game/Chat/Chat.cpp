...
ChatCommand * ChatHandler::getCommandTable()
{
    ...
    static ChatCommand commandTable[] =
    {
        ...
        { "weekreset",      SEC_ADMINISTRATOR,  false, OldHandler<&ChatHandler::HandleWeekResetCommand>,           "", NULL },
        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            if (!AccountMgr::IsPlayerAccount(table[i].SecurityLevel))
            {
                ...
                if (m_session)
                {
                    ...
                    sLogMgr->WriteGmCommand(m_session->GetAccountId(), "Command: %s [Player: %s (Account: %u) X: %f Y: %f Z: %f Map: %u Selected %s: %s (GUID: %u)]",
