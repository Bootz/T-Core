...
void World::LoadConfigSettings(bool reload)
{
    ...
    {
        sLog->outError("MaxArenaPoints (%i) can't be negative. Set to 0.", m_int_configs[CONFIG_MAX_JUSTICE_POINTS]);
        m_int_configs[CONFIG_MAX_JUSTICE_POINTS] = 0;
    }

    m_int_configs[CONFIG_START_JUSTICE_POINTS] = sConfig->GetIntDefault("StartArenaPoints", 0);
    if (int32(m_int_configs[CONFIG_START_JUSTICE_POINTS]) < 0)
    {
        sLog->outError("StartArenaPoints (%i) must be in range 0..MaxArenaPoints(%u). Set to %u.",
            m_int_configs[CONFIG_START_JUSTICE_POINTS], m_int_configs[CONFIG_START_JUSTICE_POINTS], 0);
        m_int_configs[CONFIG_START_JUSTICE_POINTS] = 0;
    }
    else if (m_int_configs[CONFIG_START_JUSTICE_POINTS] > m_int_configs[CONFIG_START_JUSTICE_POINTS])
    {
        sLog->outError("StartArenaPoints (%i) must be in range 0..MaxArenaPoints(%u). Set to %u.",
            m_int_configs[CONFIG_START_JUSTICE_POINTS], m_int_configs[CONFIG_START_JUSTICE_POINTS], m_int_configs[CONFIG_START_JUSTICE_POINTS]);
        m_int_configs[CONFIG_START_JUSTICE_POINTS] = m_int_configs[CONFIG_START_JUSTICE_POINTS];
    }
    ...
    if (reload)
    {
        ...
        m_int_configs[CONFIG_EXPANSION] = sConfig->GetIntDefault("Expansion", MAX_EXPANSION);
    ...
    m_bool_configs[CONFIG_AUTO_WEEK_RESET]                           = sConfig->GetBoolDefault("Arena.AutoWeekReset", true);
...
void World::SetInitialWorldSettings()
{
    ...
    LogonDatabase.PExecute("INSERT INTO uptime (realmid, starttime, startstring, uptime, revision) VALUES('%u', " UI64FMTD ", '%s', 0, '%s')",
         nodeID, uint64(m_startTime), isoDate, _FULLVERSION);
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_UPTIME].Passed())
    {
        ...
        m_timers[WUPDATE_UPTIME].Reset();
        LogonDatabase.PExecute("UPDATE uptime SET uptime = %u, maxplayers = %u WHERE realmid = %u AND starttime = " UI64FMTD, tmpDiff, maxOnlinePlayers, nodeID, uint64(m_startTime));
    }
...
void World::_UpdateRealmCharCount(PreparedQueryResult resultCharCount)
{
    if (resultCharCount)
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGON_DEL_REALMCHARACTERS);
        stmt->setUInt32(0, accountId);
        stmt->setUInt32(1, nodeID);
        LogonDatabase.Execute(stmt);

        stmt = LoginDatabase.GetPreparedStatement(LOGON_ADD_REALMCHARACTERS);
        stmt->setUInt32(0, charCount);
        stmt->setUInt32(1, accountId);
        stmt->setUInt32(2, nodeID);
        LogonDatabase.Execute(stmt);
    }
}
...
{
    QueryResult result = LogonDatabase.PQuery("SELECT allowedSecurityLevel from nodelist WHERE id = '%d'", nodeID);
    if (result)
